    ...
    <div class="row">
        ...
        <div class="smart-box-head">
            <h2 class="light-title title" style="margin-bottom: 0"><img :src="this.gif"
                ...
                style="vertical-align: sub;"> 为你推荐</span></h2>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2" v-for="movie in recommend1" :key="movie.pk">
                            <div class="video-item  img-desc">
                                ...
                                <div class="item-head"><h5><a @click="goDetail(movie.pk)">{{movie.name}}</a></h5></div>
                            </div>
                        </div>
...
<script>
    export default {
        ...
        created() {
            ...
            this.$axios({
                url: this.$settings.base_url + 'home/recommend/',
                method: 'get',
            }).then(response => {
                // window.console.log(response);
                this.recommend1 = response.data.splice(0, rowShow);
                this.recommend2 = response.data.splice(0, rowShow);
                // window.console.log(this.recommend1, this.recommend2, rowShow, rowShow*2)
            });
            this.$axios({
                url: this.$settings.base_url + 'home/gif/tvplay/',
                method: 'get'
            }).then(response => {
                // window.console.log(response);
